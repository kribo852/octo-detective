	...
	for i = 1,size do
		...
		for j = 1,size do
			if not ingame.obstacles[i][j] and not ingame.clue_handler.collision_with_clue(i, j)
				...
				ingame.obstacles[i][j] = "tree"..love.math.random(5)
			end
		end
	end
...
local function draw_clues()
	local to_be_drawn_on_ground_clue_positions = ingame.clue_handler.visible_on_the_ground()
	local detective_position = ingame.info_share.game_info.detective_position()

	for _,clue in ipairs(to_be_drawn_on_ground_clue_positions) do
		local image_name = clue.display_on_ground_image or clue.image

		local draw_x,draw_y = ingame.d_p_c.calc_start(detective_position.x, detective_position.y, clue.pos_x, clue.pos_y)

		ingame.image_map.get_draw_function(image_name)(draw_x, draw_y, 3, 3)
	end
end
...
function ingame.clue_summary_image_getter(image_name)
 	return ingame.image_map.get_draw_function(image_name)
end
